"""
PM33 Multi-Tenancy Testing Agent - FINAL CORRECTED Tenant Isolation Tests
Fixed session variable handling for proper RLS validation
"""
...
class TenantIsolationTestsFinal:
    """
    FINAL CORRECTED tenant isolation testing with proper session variable handling
    """
    ...
    async def test_database_row_level_security(self) -> Dict[str, Any]:
        """
        FINAL CORRECTED TEST: Verify PostgreSQL RLS properly isolates tenant data
        """
        ...
        try:
            ...
            async with self.database_pool.acquire() as connection:
                ...
                # Use session-persistent setting (false parameter)
                ...
                if current_tenant != str(tenant_a["tenant_id"]):
                    test_result["critical_failures"].append({
                        "type": "TENANT_CONTEXT_ERROR",
                        "description": f"Failed to set tenant context. Expected: {tenant_a['tenant_id']}, Got: {current_tenant}"
                    })
                ...
                test_result["details"].append({
                    "action": "verified_tenant_a_can_access_own_data",
                    "records_accessible": len(tenant_a_data)
                })
                ...
                if current_tenant != str(tenant_b["tenant_id"]):
                    test_result["critical_failures"].append({
                        "type": "TENANT_CONTEXT_ERROR",
                        "description": f"Failed to switch tenant context. Expected: {tenant_b['tenant_id']}, Got: {current_tenant}"
                    })
                    test_result["status"] = "FAILED"
                    return test_result
                ...
                if len(cross_tenant_data) > 0:
                    ...
                    test_result["details"].append({
                        "action": "verified_cross_tenant_access_blocked",
                        "result": "✅ Tenant B cannot access Tenant A data"
                    })
                ...
                if len(no_context_data) > 0:
                    ...
                    test_result["status"] = "FAILED"
                    logger.error("🚨 RLS FAILURE: Data accessible without tenant context!")
                else:
                    logger.info("✅ RLS WORKING: No data accessible without tenant context")
                    test_result["details"].append({
    ...
    async def test_oauth_credential_isolation(self) -> Dict[str, Any]:
        """
        Test OAuth credential isolation between tenants with corrected session handling
        """
        ...
        try:
            ...
            async with self.database_pool.acquire() as connection:
                # Set Tenant A context (session-persistent)
                await connection.execute(
                    "SELECT set_config('app.current_tenant_id', $1, false)",
                    str(tenant_a["tenant_id"])
                )
                ...
                test_result["details"].append({
                    "action": "verified_tenant_a_credential_access",
                    "credentials_accessible": len(tenant_a_creds)
                })
                ...
                if len(cross_tenant_creds) > 0:
                    ...
                    test_result["details"].append({
                        "action": "verified_cross_tenant_credential_blocked",
                        "result": "✅ Tenant B cannot access Tenant A credentials"
                    })
    ...
    async def run_comprehensive_isolation_tests(self) -> Dict[str, Any]:
        ...
        report = {
            "test_suite": "tenant_isolation_validation_final",
            ...
            "security_assessment": {
                ...
                "session_variable_handling": "CORRECTED"
            },
            "recommendations": [
                "✅ All critical security tests passed - Production Ready" if len(all_critical_failures) == 0 else "❌ Fix critical failures before production",
                "✅ Multi-tenant architecture validated for Phase 1 deployment" if len(all_critical_failures) == 0 else "❌ Security issues must be resolved",
                "✅ Row-Level Security policies working correctly" if len(all_critical_failures) == 0 else "❌ RLS policies require fixes"
            ]
        }
...
async def run_final_tenant_isolation_tests(pool: asyncpg.Pool) -> Dict[str, Any]:
    """Run final corrected tenant isolation tests"""
    testing_agent = TenantIsolationTestsFinal(pool)
    report = await testing_agent.run_comprehensive_isolation_tests()
    return report

async def main():
    """Main test execution"""
    logger.info("🚀 Starting FINAL CORRECTED comprehensive tenant isolation testing...")
    ...
    try:
        report = await run_final_tenant_isolation_tests(pool)
